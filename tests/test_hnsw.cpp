...
TEST(DistanceFunctions, BasicKernels) {
    ...
    EXPECT_DOUBLE_EQ(l2_scalar(a, b, 3), 14.0f);
    EXPECT_DOUBLE_EQ(ip_scalar(a, b, 3), 28.0f);
    ...
    EXPECT_DOUBLE_EQ(
        computeDistance(DistanceType::INNER_PRODUCT, a, b, 3),
        ip_scalar(a, b, 3)
    );

    EXPECT_NEAR(
        computeDistance(DistanceType::COSINE, a, b, 3),
        cosine_scalar(a, b, 3),
        1e-5f
    );
}

TEST(HnswCPU, InnerProductAndCosineModes) {
    ...
    HnswCPU indexIP(8, 100, 123, DistanceType::INNER_PRODUCT);
    indexIP.create(data);
    ...
    for (size_t i = 0; i < 5; ++i) {
        ...
        auto resIP = indexIP.search(data[i], K);
        auto resCos = indexCos.search(data[i], K);

        ASSERT_EQ(resIP.size(), K);
