import hnswlib
import numpy as np
import time

scenarios = [
    (1000, 128, 10),
    (5000, 64, 10),
    (10000, 32, 5)
]

print()
print(f"{'Dataset':>6} | {'Dim':>3} | {'K':>2} | {'Build (s)':>9} | {'Query (us)':>10} | {'Recall':>6}")
print("-" * 60)

for N, DIM, K in scenarios:
    data = np.random.rand(N, DIM).astype(np.float32)

    index = hnswlib.Index(space='l2', dim=DIM)
    index.init_index(max_elements=N, ef_construction=200, M=16)

    t1 = time.time()
    index.add_items(data)
    t2 = time.time()
    build_time = t2 - t1

    index.set_ef(200)
    q = data[:min(100, N)]

    t3 = time.time()
    labels, _ = index.knn_query(q, k=K)
    t4 = time.time()
    query_time = (t4 - t3) / len(q) * 1e6  # microseconds per query

    # Recall = fraction of queries where true neighbor is in top-K
    recall = np.mean([i in labels[i] for i in range(len(q))])

    print(f"{N:6} | {DIM:3} | {K:2} | {build_time:9.4f} | {query_time:10.3f} | {recall:6.4f}")

print()